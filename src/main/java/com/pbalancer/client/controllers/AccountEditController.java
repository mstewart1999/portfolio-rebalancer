...
import com.pbalancer.client.controllers.cells.AccountTypeListCell;
...
public class AccountEditController extends BaseController<Account,Account>
{
    ...
    void initialize()
    {
        ...
        t.getColumns().get(6).setCellValueFactory(new PropertyValueFactory<>("bestValueTmstpAsString"));
    ...
    private void populateTotalValue()
    {
        ...
        if(acct.getLastValueTmstp() != null)
        {
            valueAsOfLabel.setText(
                String.format("(as of %s)", acct.getLastValueTmstpRange()));
        }
        else
        {
            valueAsOfLabel.setText("");
